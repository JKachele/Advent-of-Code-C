...
void insertOrModify(node **cur, u64 address, u64 value) {
        ...
        if ((*cur)->address < address) {
                insertOrModify(&((*cur)->right), address, value);
                return;
        }

        // If curNode's address is greater than address, go left
        if ((*cur)->address > address) {
                insertOrModify(&((*cur)->left), address, value);
                return;
        }
}
...
void part1(struct input input) {
        ...
        tll_foreach(instrs, it) {
                ...
                insertOrModify(&root, curInstr.address, value);
        }
